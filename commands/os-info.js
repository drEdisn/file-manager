...
export default async function getOSInfo (input) {
  if (getCmdChunk(input, 1) == '--EOL') {
    console.log(os.EOL.split(''));
  } else if (getCmdChunk(input, 1) == '--cpus') {
    console.log(`Overal amount: ${os.cpus().length}`);
    os.cpus().forEach(cp => {
      console.log(`Model: ${cp.model.split('CPU')[0]}\nClock rate: ${(cp.speed / 1000).toFixed(2)} GHz`)
    });
  } else if (getCmdChunk(input, 1) == '--homedir') {
    console.log(os.homedir());
  } else if (getCmdChunk(input, 1) == '--username') {
    console.log(os.userInfo().username);
  } else if (getCmdChunk(input, 1) == '--architecture') {
    console.log(os.arch());
  } else {
